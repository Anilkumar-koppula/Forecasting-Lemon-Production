...
data_ts <- ts(data$BANGALURU, start = c(2002, 1), end = c(2022, 12), frequency = 12)
data_ts
BANGALORE<- ts(data$BANGALURU, start = c(2002, 1), end = c(2020, 12), frequency = 12)
...
plot(acf_result, main = "Bangalore")
# Compute autocorrelation function
pacf_result <- pacf(diff_ts)

# Plot autocorrelation function with title
plot(pacf_result, main = "Bangalore")
...
library(forecast)
model_2 = Arima(BANGALURU,order =c(1,1,2))
...
#PLOTTING FORECAST
autoplot(forecast_2,main = "Bangalore",xlab="Year", ylab = "Production in Tons")
