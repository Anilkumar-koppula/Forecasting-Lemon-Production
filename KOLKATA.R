...
KOLKATA <- ts(data$KOLKATA, start = c(2002, 1), end = c(2022, 12), frequency = 12)
KOLKATA
...
plot(acf_result, main = "Kolkata")
# Compute autocorrelation function
pacf_result <- pacf(diff_ts)

# Plot autocorrelation function with title
plot(pacf_result, main = "Kolkata")
# AUTO ARIMA

auto_arima =auto.arima(KOLKATA)

#FORECASTING FOR AUTO.ARIMA
library(forecast)
forecast = forecast(auto_arima,h=24)
forecast
autoplot(forecast,main = "Kolkata",xlab="Year", ylab = "Production in Tons")
